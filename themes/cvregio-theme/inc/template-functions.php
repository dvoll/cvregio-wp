...
{
	if (is_category()) {
		$title = __('Beiträge in der Kategorie: ', 'twentynineteen') . '<span class="page-description">' . single_term_title('', false) . '</span>';
	} elseif (is_tag()) {
		$title = __('Beiträge mit dem Stichwort: ', 'twentynineteen') . '<span class="page-description">' . single_term_title('', false) . '</span>';
	} elseif (is_author()) {
		$title = __('Beiträge von: ', 'twentynineteen') . '<span class="page-description">' . get_the_author_meta('display_name') . '</span>';
	} elseif (is_year()) {
		$title = __('Beiträge aus dem Jahr: ', 'twentynineteen') . '<span class="page-description">' . get_the_date(_x('Y', 'yearly archives date format', 'twentynineteen')) . '</span>';
	} elseif (is_month()) {
		$title = __('Beiträge aus dem Monat: ', 'twentynineteen') . '<span class="page-description">' . get_the_date(_x('F Y', 'monthly archives date format', 'twentynineteen')) . '</span>';
	} elseif (is_day()) {
		$title = __('Beiträge vom: ', 'twentynineteen') . '<span class="page-description">' . get_the_date() . '</span>';
	} elseif (is_post_type_archive()) {
		$title = __('Beiträge vom Typ: ', 'twentynineteen') . '<span class="page-description">' . post_type_archive_title('', false) . '</span>';
	} elseif (is_tax()) {
		$tax = get_taxonomy(get_queried_object()->taxonomy);
		/* translators: %s: Taxonomy singular name */
		$title = sprintf(esc_html__('%s Beiträge:', 'twentynineteen'), $tax->labels->singular_name);
	} else {
		$title = __('Beiträge:', 'twentynineteen');
	}
	return $title;
}
...
function twentynineteen_add_ellipses_to_nav($nav_menu, $args)
{
	...
	if ('menu-1' === $args->theme_location) :
		...
		$nav_menu .= '<span class="screen-reader-text">' . esc_html__('Mehr', 'twentynineteen') . '</span>';
		...
		$nav_menu .= esc_html__('Zurück', 'twentynineteen');
		$nav_menu .= '</button>';
		$nav_menu .= '</li>';
		$nav_menu .= '</ul>';
		$nav_menu .= '</li>';
