...
wp_enqueue_style('cv-frontend-vendor-css');
  `;
    ...
    const commonCss = await getFileAndCopy('chunk-common', 'css', pluginDir + appDist);
    content += `
wp_register_style(
  'cv-frontend-common-css',
  plugins_url( '${appDist + commonCss}', dirname( __FILE__ ) ),
  array( 'wp-editor' )
);
...
wp_enqueue_style(
  ...
  `;
    ...
    content += `
wp_register_script('cv-frontend-vendor-js', plugins_url( '${appDist +
        vendorJs}', dirname( __FILE__ )), array(), '1.0', true );
wp_enqueue_script('cv-frontend-vendor-js');
`;

    const commonJs = await getFileAndCopy('chunk-common', 'js', pluginDir + appDist);
    content += `
wp_register_script('cv-frontend-common-js', plugins_url( '${appDist +
        commonJs}', dirname( __FILE__ )), array(), '1.0', true );
wp_enqueue_script('cv-frontend-common-js');
`;
    const appJs = await getFileAndCopy('app', 'js', pluginDir + appDist);
    content += `
wp_enqueue_script('cv-frontend-app-js', plugins_url( '${appDist +
        appJs}', dirname( __FILE__ )), array(), '1.0', true );
  `;
